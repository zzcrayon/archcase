...
- 建筑案例管理：支持新增、编辑、删除和详情查看，适合长期整理建筑案例库。
- 搜索与分类筛选：可以按建筑名称、建筑师、城市、国家、类型、标签和文本内容快速查找案例。
- 评分系统：从空间组织、材料表达、结构创新、场地回应、设计启发 5 个维度记录案例价值。
- 数据统计面板：自动统计案例数量、类型分布、平均评分、高频标签和高分案例。
- JSON 导入导出：支持把案例库导出备份，也可以从 JSON 文件恢复数据。
- 本地图片上传：支持上传本地建筑图片，并在卡片、详情页和大图预览中展示。
- Vercel 在线部署：项目已经部署上线，可作为前端作品集项目直接访问展示。
